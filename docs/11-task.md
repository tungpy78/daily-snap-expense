...
*   **T-14.2.5**: React Navigation và Camera-first Home Shell
    ...
    *   **Mô tả**: Tạo `NavigationContainer`, `RootNavigator`, `AuthStack` và `AppTabs`. Sau khi đăng nhập, người dùng vào Home mặc định. Home là màn camera-first thiết kế dạng vertical page-snapping (Locket-style): Trang 1 hiển thị camera card bo góc nhỏ gọn ở phần trên, Trang 2 hiển thị Feed khi vuốt xuống. Không tạo tab Timeline riêng ở bước này. App navigation gồm 4 tab: Trang chủ, Kỷ niệm, Chi tiêu và Cá nhân. Chụp ảnh, nén ảnh cục bộ và mở SnapComposerModal trực tiếp tại Home. Xóa nút test camera tạm, dọn dẹp App.tsx.
    *   **Module ảnh hưởng**: Mobile Navigation Foundation, Mobile Camera/Home Feature
    *   **Test**: App mở được, restore session hoạt động, login/logout đúng flow, sau đăng nhập vào Home mặc định, Home hiển thị khung camera bo góc nhỏ gọn (không có vệt đen), chụp ảnh trực tiếp được và hiện modal composer, overscroll không bị giật, vuốt chuyển đổi giữa Camera và Feed mượt mà, điều hướng tới Memories/Expenses/Profile hoạt động.
